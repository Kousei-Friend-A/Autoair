...
class FFEncoder:
    ...
    async def progress(self):
        ...
        while self.__proc and not self.is_cancelled:
            async with aiopen(self.__prog_file, 'r') as p:
                text = await p.read()
            if text:
                time_done = floor(int(findall(r"out_time_ms=(\d+)", text)[-1]) / 1_000_000) if findall("out_time_ms=(\d+)", text) else 1
...
<blockquote>📂 <b>File(s) Encoded:</b> <code>{Var.QUALS.index(self.__qual)} / {len(Var.QUALS)}</code></blockquote>"""
                ...
                if findall(r"progress=(\w+)", text)[-1] == 'end':
                    break
            await asleep(2)  # Reduced sleep time for more frequent updates
    ...
    async def start_encode(self):
        ...
        await gather(create_task(self.progress()), self.__proc.wait())
